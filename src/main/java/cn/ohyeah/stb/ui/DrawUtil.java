...
public class DrawUtil {
	...
	public static void drawAdaptiveButton(SGraphics g, Image btnBg, int btnSrcX, int btnSrcY,
		...
		if (btnSw > btnW) {
			...
			if (btnSw <= (btnFillW + btnFillW)) {
				g.drawRegion(btnBg, btnSrcX + (btnW - (btnSw - btnFillW)), btnSrcY, btnSw
						- btnFillW, btnH, 0, sx, sy, 20);
			} else {
				int w = btnSw - btnFillW - btnFillW;
				while (w > 0) {
					if (w > btnMidW) {
						g.drawRegion(btnBg, btnSrcX + btnBorder, btnSrcY, btnMidW, btnH, 0, sx, sy,
								20);
						sx += btnMidW;
						w -= btnMidW;
					} else {
						g.drawRegion(btnBg, btnSrcX + btnBorder, btnSrcY, w, btnH, 0, sx, sy, 20);
						sx += w;
						w -= w;
					}
				}
				g.drawRegion(btnBg, btnSrcX + btnBorder, btnSrcY, btnFillW, btnH, 0, sx, sy, 20);
			}
		} else {
	...
	public static int drawNumber(SGraphics g, Image img, int num, int x, int y, int gap) {
		...
		for (int i = 0; i < number.length(); ++i) {
			g.drawRegion(img, imgW * Character.digit(number.charAt(i), 10), 0, imgW, imgH, 0, sx,
					sy, 20);
			sx += imgW + gap;
		}
		return sx - x;
