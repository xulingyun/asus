...
public class StateRecharge {
	...
	private static final String[] imagePaths = { "/business/recharge-bg.jpg",
			"/business/recharge-title.png", "/business/exchange-title.png",
			"/business/confirm-bg.jpg", "/business/checked.png", "/business/unchecked.png",
			"/business/ok0.png", "/business/cancel0.png", "/business/back0.png",
			"/business/recharge0.png", "/business/exchange0.png", "/business/password-bg.png", };

	private static char[][] inputChars = { { '0' }, { '1' }, { '2', 'a', 'A', 'b', 'B', 'c', 'C' },
			{ '3', 'd', 'D', 'e', 'E', 'f', 'F' }, { '4', 'g', 'G', 'h', 'H', 'i', 'I' },
			{ '5', 'j', 'J', 'k', 'K', 'l', 'L' }, { '6', 'm', 'M', 'n', 'N', 'o', 'O' },
			{ '7', 'p', 'P', 'q', 'Q', 'r', 'R', 's', 'S' }, { '8', 't', 'T', 'u', 'U', 'v', 'V' },
			{ '9', 'w', 'W', 'x', 'X', 'y', 'Y', 'z', 'Z' } };
	...
	private void drawPassword(SGraphics g, int x, int y, int w, int h, boolean drawCursor,
			boolean drawChar) {
		g.setColor(0Xffff00);
		Font font = g.getFont();
		...
		if (drawChar) {
			...
			g.setColor(0Xffff00);
			g.drawChar(pwdChars[pwdCharIndex], sx + (22 - font.charWidth(pwdChars[pwdCharIndex]))
					/ 2, sy, 20);
		}
	}
	...
	private void showInputPwd(SGraphics g) {
		...
		if (subState == SUB_STATE_INPUT_PWD_VIEW) {
			if (pwdGroupIndex == 1) {
				if (pwdBtnIndex == 0) {
					DrawUtil.drawRect(g, bgX + 120, bgY + 280, ok.getWidth(), ok.getHeight(), 2,
							0XFFFF00);
				} else {
					DrawUtil.drawRect(g, bgX + 266, bgY + 280, cancel.getWidth(),
							cancel.getHeight(), 2, 0XFFFF00);
				}
			}
		}
	}
	...
	private void showConfirm(SGraphics g) {
		...
		String productName = engineService.getRechargeCommand() + rechargeAmount
				* engineService.getExpendCashToAmountRatio() + engineService.getExpendAmountUnit();
		// String productName =
		// "<<"+engineService.getProductName()+">>"+engineService.getRechargeCommand();
		Font font = g.getFont();
		...
		String ss = null;
		if (curPayType == 0) {
			ss = rechargeAmount * engineService.getSubscribeCashToAmountRatio()
					+ engineService.getSubscribeAmountUnit();
		} else {
			ss = rechargeAmount * engineService.getCashToPointsRatio()
	...
	private void showSelectAmount(SGraphics g) {
		g.setColor(43, 39, 36);
		// g.fillRect(0, 0, engine.getScreenWidth()-Configurations.Abs_Coords_X,
		// engine.getScreenHeight()-Configurations.Abs_Coords_Y);
		...
		for (int i = 0; i < amountList.length; ++i) {
			...
			ss = amountList[i] * engineService.getRechargeRatio()
					+ engineService.getExpendAmountUnit();
			g.setColor(0XFFFF00);
		...
		if (groupIndex == 1) {
			DrawUtil.drawRect(g, btnX, amountY + 2, recharge.getWidth(), recharge.getHeight(), 2,
					0XFFFF00);
				...
				if (pwdBtnIndex == 0) {
					...
					try {
						if (curPayType == 0) {
							...
							sw.recharge(rechargeAmount * engineService.getCashToPointsRatio(),
									SubscribePayType.PAY_TYPE_POINTS, password);
						}
						if (sw.isServiceSuccessful()) {
							...
							resultMsg = engineService.getRechargeCommand() + "失败，原因："
									+ sw.getMessage();
						}
					} catch (Exception e) {
						e.printStackTrace();
	...
	private boolean isPasswordError(String msg) {
		...
		if (msg.indexOf("密码") >= 0) {
			if (msg.indexOf("错误") > 0 || msg.indexOf("校验失败") > 0) {
				pwdError = true;
			}
		}
		return pwdError;
				...
				try {
					if (curPayType == 0) {
						if (engineService.isRechrageSuccess
								&& !Configurations.getInstance().isTelcomOperatorsTelcomah()) {
							sw.recharge(rechargeAmount, SubscribePayType.PAY_TYPE_BILL,
									engineService.passWord);
						} else {
							sw.recharge(rechargeAmount, SubscribePayType.PAY_TYPE_BILL, "-250");
						}
					} else {
						if (engineService.isRechrageSuccess
								&& !Configurations.getInstance().isTelcomOperatorsTelcomah()) {
							sw.recharge(rechargeAmount * engineService.getCashToPointsRatio(),
									SubscribePayType.PAY_TYPE_POINTS, engineService.passWord);
						} else {
							sw.recharge(rechargeAmount * engineService.getCashToPointsRatio(),
									SubscribePayType.PAY_TYPE_POINTS, "-250");
						}
					}
					if (sw.isServiceSuccessful()) {
						resultMsg = engineService.getRechargeCommand() + "成功";
		...
		if (key.containsAndRemove(KeyCode.UP)) {
			...
			if (groupIndex == 0) {
				...
				if (engineService.isSupportRecharge()) {
					...
					if (curPayType == 1) {
						if (engineService.getAvailablePoints() < rechargeAmount
								* engineService.getCashToPointsRatio()) {
							PopupText pt = UIResource.getInstance().buildDefaultPopupText();
